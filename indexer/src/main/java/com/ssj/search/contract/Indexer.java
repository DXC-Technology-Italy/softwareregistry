	    ...
	    if (Files.isDirectory(path)) {
	        ...
	        Files.walkFileTree(path, new SimpleFileVisitor<Path>() {
				...
				public FileVisitResult preVisitDirectory( final Path dir, final BasicFileAttributes attrs ) throws IOException {
					if (EnumUtils.isValidEnum(DirectoryToSkip.class, dir.getFileName().toString().replace(".",""))) return FileVisitResult.SKIP_SUBTREE;

					return FileVisitResult.CONTINUE; //or CONTINUE for recursive processing
				}
